...
/**
 * Evaluates the derived point-in-time availability and scheduling eligibility of a technician by employeeId.
 *
 ...
 *   - Returns `TechnicianAvailabilityCheck | null` if employee is not found or belongs to another workspace.
 ...
 */
export async function getTechnicianAvailabilityCheckByEmployee(
    workspaceId: string,
    employeeId: string,
    input: unknown,
    ...
    const activeRecurring =
        profile?.technicianAvailabilities.filter(
            (a) => a.status === "ACTIVE",
        ) ?? [];
    ...
    if (!isInvalidInterval && parsedInterval && profile) {
